...
export default function HomeClientPage({ locale }: { locale: Locale }) {
  ...
  const abilities = [
    ...
    { icon: Globe, title: "Многоязычность", desc: "Работает на 15+ языках в одной системе" },
  ...
  const faqItems = [
    {
      q: t.home?.faq_q1 || "Как быстро можно запустить AI?",
      a: t.home?.faq_a1 || "Типовое внедрение занимает 7-14 дней. Это включает интеграцию с вашими системами, настройку сценариев и тестовый период. Срок можно сократить до 3-4 дней для типовых сценариев.",
    },
    {
      q: t.home?.faq_q2 || "Какие системы поддерживаются?",
      a: t.home?.faq_a2 || "Мы интегрируемся с amoCRM, Bitrix24, Leadvertex, 1С, Контур, Avaya, Asterisk, SIP, WebRTC. Если вашей системы нет в списке — разработаем интеграцию специально для вас.",
    },
    {
      q: t.home?.faq_q3 || "Сколько стоит?",
      a: t.home?.faq_a3 || "Цена зависит от количества звонков и нужных интеграций. Стартовый пакет с 100-500 звонками обходится от $500/месяц. Для точного расчета оставьте контакты — составим предложение за 24 часа.",
    },
    {
      q: t.home?.faq_q4 || "Может ли робот общаться на разных языках?",
      a: t.home?.faq_a4 || "Да, AI поддерживает 15+ языков. Один робот может переключаться между языками в одном разговоре или обслуживать разные страны под разные сценарии.",
    },
    {
      q: t.home?.faq_q5 || "Что если AI не поймёт клиента?",
      a: t.home?.faq_a5 || "Система автоматически анализирует степень уверенности. При низком уровне понимания робот просит повторить или предлагает перейти на менеджера. Вероятность ошибок менее 2%.",
    },
    {
      q: t.home?.faq_q6 || "Как считаются звонки?",
      a: t.home?.faq_a6 || "За звонок тарифицируется минута беседы с AI, включая все переводы на менеджеров. Входящие и исходящие звонки тарифицируются отдельно. Записи разговоров и аналитика не тарифицируются.",
    },
  ]
