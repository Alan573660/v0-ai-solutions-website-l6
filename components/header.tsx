      ...
      <div className="container mx-auto px-3 sm:px-4">
        ...
        {isMenuOpen && (
          <div
            ref={mobileMenuRef}
            id="mobile-menu"
            className="md:hidden absolute inset-x-0 top-full bg-background border-t shadow-xl max-h-[calc(100dvh-3.5rem)] overflow-hidden"
            role="navigation"
          ...
          >
            <div className="h-full flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto overscroll-contain">
                <nav className="py-2">
                  ...
                  <div className="px-4 py-3 space-y-1">
                    {/* Hero Link - Business Automation */}
                    <Link
                      href={`/${locale}/solutions/business-automation`}
                      onClick={() => setIsMenuOpen(false)}
                      className="flex items-center gap-3 p-3 rounded-2xl bg-gradient-to-r from-primary/10 via-violet-500/10 to-primary/5 border border-primary/20"
                    >
                      <div className="h-10 w-10 rounded-xl bg-primary flex items-center justify-center">
                        <Sparkles className="h-5 w-5 text-primary-foreground" />
                      </div>
                      <div>
                        <p className="font-semibold text-sm">{locale === "ru" ? "Автоматизация бизнеса ИИ" : "AI Business Automation"}</p>
                        <p className="text-xs text-muted-foreground">{locale === "ru" ? "Комплексные решения" : "Complete solutions"}</p>
                      </div>
                    ...
                    >
                      ...
                      <span className="font-medium text-sm">{t("solutions.voiceSalesManager")}</span>
                    </Link>

                    {/* M2 Agents */}
                    <Link
                      href={`/${locale}/solutions/m2-agents`}
                      onClick={() => setIsMenuOpen(false)}
                      className="flex items-center gap-3 p-3 rounded-xl hover:bg-muted/50 transition-colors"
                    >
                      ...
                      <span className="font-medium text-sm">{t("solutions.m2Agents")}</span>
                    </Link>
                  </div>
                  ...
                  <div className="px-4 space-y-1">
                    ...
                    >
                      <span className="text-sm font-medium text-muted-foreground">{locale === "ru" ? "Все решения" : "All Solutions"}</span>
                    ...
                    {expandedSection === "solutions" && (
                      <div className="pl-3 pb-2 space-y-0.5">
                        {solutions.filter(s => !s.highlight).map((item) => (
                          <Link
                            key={item.href}
                            href={item.href}
                            onClick={() => setIsMenuOpen(false)}
                            className="flex items-center gap-2.5 py-2.5 px-3 rounded-lg text-sm text-foreground/80 hover:bg-muted/40 transition-colors"
                          >
                    ...
                    >
                      <span className="text-sm font-medium text-muted-foreground">{locale === "ru" ? "AI решения" : "AI Solutions"}</span>
                      <ChevronDown className={cn(
                        "h-4 w-4 text-muted-foreground transition-transform duration-200",
                    ...
                    {expandedSection === "ai" && (
                      <div className="pl-3 pb-2 space-y-0.5">
                        {[...aiAutomationMain.slice(1), ...aiAutomationOperators.slice(0, 2), ...aiAutomationSales.slice(0, 2)].map((item) => (
                          <Link
                            key={item.href}
                            href={item.href}
                            onClick={() => setIsMenuOpen(false)}
                            className="block py-2.5 px-3 rounded-lg text-sm text-foreground/80 hover:bg-muted/40 transition-colors"
                          >
                            {item.name}
                          </Link>
                        ))}
                        <Link
                          href={`/${locale}/automation-business`}
                          onClick={() => setIsMenuOpen(false)}
                          className="block py-2.5 px-3 rounded-lg text-sm text-primary font-medium hover:bg-primary/5 transition-colors"
                        >
                          {locale === "ru" ? "Все AI решения →" : "All AI Solutions →"}
                        </Link>
                      </div>
                    )}
                    ...
                    >
                      <span className="text-sm font-medium text-muted-foreground">{t("nav.platform")}</span>
                      <ChevronDown className={cn(
                        "h-4 w-4 text-muted-foreground transition-transform duration-200",
                  ...
                  <div className="px-4 py-2">
                    <div className="flex items-center gap-1">
                      <Link
                        href={`/${locale}/cases`}
                        onClick={() => setIsMenuOpen(false)}
                        className="flex-1 py-2.5 text-center text-sm font-medium rounded-lg hover:bg-muted/40 transition-colors"
                      >
                        {t("nav.cases")}
                      </Link>
                      <Link
              ...
              <div className="shrink-0 border-t bg-background/95 backdrop-blur-sm px-4 py-3 safe-area-inset-bottom">
                <div className="flex gap-3">
                  <Button variant="outline" className="flex-1 h-12 text-sm bg-transparent" asChild>
